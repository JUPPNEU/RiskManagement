    ...
    current.forEach(curItem => {
      ...
      if (targetItem) {
        curItem.ext.forEach(ext => {
          ...
          if (targetExt) { 
            for (const key in ext.propeties) {
              if (ext.propeties.hasOwnProperty(key)) {
                if (ext.propeties[key] === true) {
                  if (targetExt.propeties[key] !== true) {
                    if (targetExt.propeties[key]) {
                  ...
                  if (targetExt.propeties[key] !== true) {
                    if (Array.isArray(ext.propeties[key])) {
                      ext.propeties[key].forEach(v => {
                        switch (v) {
                          case KEY_ARRAY_REORDER:
                          case KEY_ARRAY_REMOVE:
                            if (!targetExt.propeties[key]) {
                              sError += `Notfound "${extName}/${key}" in ${targetItem.file}\n`;
                            } else if (!targetExt.propeties[key].includes(v)) {
                          ...
                          case KEY_ARRAY_INSERT_END:
                            if (!targetExt.propeties[key]) {
                              sError += `Notfound "${extName}/${key}" in ${targetItem.file}\n`;
                            }  else if (!targetExt.propeties[key].includes(v) && !targetExt.propeties[key].includes(KEY_ARRAY_INSERT_ANY)) {
                              sError += `Unmatch "${extName}/${key}/${v}" in ${targetItem.file}\n`;
                            }
                            break;
                          case KEY_ARRAY_INSERT_ANY:
                            if (!targetExt.propeties[key]) {
                              sError += `Notfound "${extName}/${key}" in ${targetItem.file}\n`;
                            } else if (!targetExt.propeties[key].includes(KEY_ARRAY_INSERT_ANY)) {
                              sError += `Unmatch "${extName}/${key}/${v}" in ${targetItem.file}\n`;
                            }
                            break;
                        }
    ...
    if (obj[EXTENSION_POINT_KEY]) {
      if (obj[EXTENSION_POINT_KEY] === true) { //global
        ...
        if (obj._Name) {
          report.ext.push({
            "name": obj._Name,
            "propeties": obj[EXTENSION_POINT_KEY]
          });
        } else {
          report.ext.push({
            "propeties": obj[EXTENSION_POINT_KEY]
        ...
        if (isRule) {
          ...
          for (const key in item) {
            if (item.hasOwnProperty(key) && key !== "_Name") {
              if (existingItem) {
                existingItem.propeties[key] = instance.getMergemapChange(item[key]);
              } else {
                let newItem = {
                  "propeties": {}
                };
                ...
                newItem.propeties[key] = instance.getMergemapChange(item[key]);
